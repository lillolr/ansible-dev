...
DOCUMENTATION = r'''
---
module: dellemc_isilon_smb

version_added: '1.1.1'

short_description: Manage SMB shares on Dell EMC Isilon.
description:
- Managing SMB share on Isilon.
...
extends_documentation_fragment:
  - dellemc.isilon.dellemc_isilon.isilon
...
options:
  ...
  path:
    ...
    - The path of the SMB share. This parameter will be mandatory only
      for create operation. This is absolute path for System Access Zone and
      relative path for non System Access Zone.
    type: str
  access_zone:
    description:
    - Access zone which contains this share. If not specified it will be
      considered as System Access Zone.
    - For non System Access Zone the effective path where the SMB is
      created will be determined by the base path of the Access Zone and
      the path provided by the user in the playbook.
    - For System Access Zone the effective path will be the absolute path
  ...
  access_based_enumeration:
    description:
    - Only enumerate files and folders for the requesting user has access to.
    type: bool
  ...
  directory_create_mask:
    description:
    - Directory create mask bits. Octal value for owner,group,others vs
      read,write,execute
    type: str
  directory_create_mode:
    description:
    - Directory create mode bits. Octal value for owner,group,others vs
      read,write,execute
    type: str
  file_create_mask:
    description:
    - File create mask bits. Octal value for owner,group,others vs
      read,write,execute
    type: str
  file_create_mode:
    description:
    - File create mode bits. Octal value for owner,group,others vs
      read,write,execute
    type: str
  state:
    description:
...
class IsilonSMB(object):
    ...
    def __init__(self):
        ...
        self.module_params.update(get_isilon_smb_parameters())
        ...
        if HAS_ISILON_SDK is False:
            self.module.fail_json(
                msg='Ansible modules for Isilon '
        ...
        self.protocol_api = self.isi_sdk.ProtocolsApi(
            self.api_client)
        LOG.info('Got instance for ProtocolsApi on Isilon ')
        self.zone_summary_api = self.isi_sdk.ZonesSummaryApi(self.api_client)
        LOG.info('Got instance for ZonesSummaryApi on Isilon ')
        self.auth_api_instance = utils.isi_sdk.AuthApi(self.api_client)
        LOG.info('Got instance for AuthApi on Isilon')
...
def main():
    """Create Isilon Smb object and perform action on it
