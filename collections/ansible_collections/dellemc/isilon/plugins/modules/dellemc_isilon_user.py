...
""" Ansible module for managing Users on Isilon"""
...
DOCUMENTATION = r'''
---
module: dellemc_isilon_user
short_description: Manage users on the Isilon Storage System
description:
- Managing Users on the Isilon Storage System includes create user,
...
options:
  ...
  provider_type:
    ...
    - Adding and removing roles is allowed for all users of
      system access zone.
  ...
  primary_group:
    description:
    - A user can be member of multiple group of which one group has
  ...
  home_directory:
    ...
    - For user in system access zone, absolute path has to be given.
    - For users in non system access zone, path relative to
      non system Access Zone's base directory has to be given.
    type: str
  shell:
    description:
...
user_details:
    ...
    contains:
        ...
        enabled:
            description:
                - Enabled is a bool variable which is used to enable or
        ...
        home_directory:
            description:
                - The directory path acts as the home directory
...
class IsilonUser(object):
    ...
    def update_user(self, auth_user_id, zone, provider, enabled,
                    ...
                    full_name, email):
        """Update the User Account details in Isilon"""
        try:
            if primary_group:
                primary_group = utils.isi_sdk.AuthAccessAccessItemFileGroup(
                    "GROUP:" + primary_group)
    ...
    def add_role_to_user(self, auth_user_id, role_name):
        """Add a Role to a User in Isilon"""
        try:
    ...
    def remove_role_from_user(self, role_member_id, role_name):
        """ Remove a Role from a User in Isilon"""
        try:
...
def get_isilon_user_parameters():
...
def main():
    """ Create Isilon User object and perform actions on it
