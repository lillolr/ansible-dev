...
"""Ansible module for managing snapshot schedules on Isilon"""
...
DOCUMENTATION = r'''
---
module: dellemc_isilon_snapshotschedule
version_added: '1.1.1'
short_description: Manage snapshot schedules on Dell EMC Isilon.
description:
- Managing snapshot schedules on Isilon.
...
options:
  ...
  path:
    ...
    - For 'System' access zone, path is absolute.
    - This parameter is required at the time of creation.
    - Modification of path is not allowed through Ansible module.
    type: str
  access_zone:
    description:
  ...
  desired_retention:
    ...
    - For large durations (beyond days/weeks), Isilon may round off the
...
class IsilonSnapshotSchedule(object):
    ...
    def __init__(self):
        ...
        self.module_params.update(get_isilon_snapshotschedule_parameters())
...
def get_isilon_snapshotschedule_parameters():
    """This method provide parameters required for the ansible snapshot
    schedule module on Isilon"""
    return dict(
...
def main():
    """ Create Isilon snapshot schedule object and perform action on it
        based on user input from playbook"""
    obj = IsilonSnapshotSchedule()
