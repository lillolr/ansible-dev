...
"""Ansible module for managing NFS Exports on DellEMC Isilon"""
...
class IsilonNfsExport(object):
    ...
    def _check_mod_field(self, field_name_playbook, field_name_isilon):
        _field_mod = False
        if self.module.params[field_name_playbook] is None:
            field_value = self.result['NFS_export_details'][field_name_isilon]
        elif self.module.params[field_name_playbook] == \
                self.result['NFS_export_details'][field_name_isilon]:
            field_value = self.result['NFS_export_details'][field_name_isilon]
        else:
    ...
    def get_isilon_nfs_parameters(self):
        return dict(
...
def main():
    ''' Create Isilon_NFS export object and perform action on it
