...
try:
    ...
    PKG_RSRC_IMPORT_SUCCESS = True
except ImportError:
    PKG_RSRC_IMPORT_SUCCESS = False
...
def pyu4v_version_check():
    try:
        if not PKG_RSRC_IMPORT_SUCCESS:
        ...
        unsupported_version_message = "PyU4V {0} is not supported by " \
                                      "this module.Minimum supported " \
                                      "version is : " \
                                      "{1} ".format(curr_version,
                                                    min_ver)
        supported_version = parse_version(curr_version) \
        ...
        unsupported_version_message = "Unable to get the PyU4V " \
...
'''
This method provide parameter required for the ansible modules on
PowerMax
options:
  unispherehost:
    description:
  ...
  verifycert:
    description:
    - Boolean value to inform system whether to verify client
      certificate or not.
  user:
    description:
...
def get_powermax_management_host_parameters():
    return dict(
        ...
        verifycert=dict(choices=[True, False], type='bool', required=True),
...
parameters:
  module_params - Ansible module parameters which contain below
                  unisphere details to establish connection on to
                  Unisphere
     - unispherehost: IP/FQDN of unisphere host.
     - universion:Version of univmax SDK.
     - verifycert: Boolean value to inform system whether to verify
...
def get_logger(module_name,
               log_file_name='dellemc_ansible_provisioning.log',
...
'''
Convert the given size to size in GB, size is restricted to 2 decimal
places
'''
