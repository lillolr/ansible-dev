...
class PowerMaxPort(object):
    ...
    def get_port_details(self):
        ...
        try:
            details = {}
            message = 'Director ID and Port ID is mandatory for' \
            ...
            for port in ports:
                ...
                details[director_id + ':' + port_id] = \
                    self.provisioning.get_director_port(director_id,
                                                        port_id)
                self.result['port_details'] = details
            return
        except Exception as e:
            error_message = 'Failed to get details of port {0}:{1} ' \
                            'with error {2}'.format(director_id,
                                                    port_id, str(e))
            LOG.error(error_message)
            self.module.fail_json(msg=error_message, **self.result)
...
def get_powermax_port_parameters():
    ...
    return dict(ports=dict(required=True, type='list'))
