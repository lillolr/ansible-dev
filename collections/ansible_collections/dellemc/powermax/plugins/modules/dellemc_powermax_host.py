...
short_description:  Manage host (initiator group) on PowerMax/VMAX
                    Storage System

description:
- Managing host on PowerMax Storage System includes create host with a
  set of initiators and host flags, add/remove initiators to/from host,
  modify host flag values, rename host and delete host
...
options:
  ...
  initiators:
    description:
    - List of Initiator WWN or IQN to be added to host or removed
      from the host.
    type: list
  state:
    description:
  ...
  initiator_state:
    ...
    - present-in-host - indicates that the initiators should exist on
                        host
    - absent-in-host - indicates that the initiators should not exist
                       on host
  ...
  new_name:
    description:
    - The new name host for renaming function. No Special Character
...
class PowerMaxHost(object):
    ...
    def _set_to_enable(self, host_flag_name, host_flag_dict):
        host_flag_dict[host_flag_name.lower()] = {'enabled': True,
                                                  'override': True}
    ...
    def create_host(self, host_name):
        ...
        if (initiator_state == 'absent-in-host'
                or initiator_state is None):
            initiators = None
        ...
        try:
            ...
            errorMsg = 'Create host {0} failed with error {1}'\
    ...
    def add_host_initiators(self, host_name, initiators):
        ...
        if len(add_list) > 0:
            try:
                ...
                errorMsg = (('Adding initiators {0} to host {1} failed'
                             ' with error {2}')
                            .format(add_list, host_name, str(e)))
                LOG.error(errorMsg)
                self.module.fail_json(msg=errorMsg)
        else:
    ...
    def remove_host_initiators(self, host_name, initiators):
        ...
        if existing_inits is None or not len(existing_inits):
            LOG.info('No initiators are present in host %s',
                     host_name)
            return False
        ...
        if len(remove_list) > 0:
            try:
                ...
                errorMsg = (('Removing initiators {0} from host {1} '
                             'failed with error {2}')
                            .format(remove_list, host_name, str(e)))
                LOG.error(errorMsg)
                self.module.fail_json(msg=errorMsg)
        else:
            LOG.info('No initiators to remove from host %s',
                     host_name)
            return False
    ...
    def rename_host(self, host_name, new_name):
        try:
            ...
            errorMsg = 'Renaming of host {0} failed with error {1}'\
                .format(host_name, str(e))
            LOG.error(errorMsg)
            self.module.fail_json(msg=errorMsg)
            return None
    ...
    def modify_host_flags(self, host_name, received_host_flags):
        current_flags = {}
        self._recreate_host_flag_dict(self.get_host(host_name),
                                      current_flags)
        ...
        if new_flags_dict == current_flags:
            ...
            try:
                ...
                errorMsg = 'Modify host {0} failed with error {1}'\
                    .format(host_name, str(e))
                LOG.error(errorMsg)
                self.module.fail_json(msg=errorMsg)
            return None
    ...
    def delete_host(self, host_name):
        ...
        A host cannot be deleted if it is associated with a masking
        view.
        """
        ...
        try:
            ...
            errorMsg = ('Delete host {0} failed with error {1}'
                        .format(host_name, str(e)))
            LOG.error(errorMsg)
            self.module.fail_json(msg=errorMsg)
    ...
    def perform_module_operation(self):
        ...
        if (state == 'present' and host
            and intiator_state == 'present-in-host'
                and initiators and len(initiators) > 0):
            LOG.info('Adding initiators to host %s', host_name)
            changed = (self.add_host_initiators(host_name,
                                                initiators)
                       or changed)

        if (state == 'present' and host
            and intiator_state == 'absent-in-host'
                and initiators and len(initiators) > 0):
            LOG.info('Remove initiators from host %s', host_name)
            changed = (self.remove_host_initiators(host_name,
                                                   initiators)
                       or changed)
        ...
        if state == 'present' and host and host_flags:
            ...
            changed = self.modify_host_flags(host_name, host_flags)\
                or changed

        if state == 'present' and host and new_name:
            if host['hostId'] != new_name:
                LOG.info('Renaming host %s to %s',
                         host_name, new_name)
        ...
        if state == 'absent' and host:
            ...
            changed = self.delete_host(host_name)\
                or changed
