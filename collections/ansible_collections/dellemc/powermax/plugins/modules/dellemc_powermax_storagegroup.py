...
short_description:  Manage storage groups on PowerMax/VMAX Storage
                    System
...
LOG = utils.get_logger('dellemc_powermax_storagegroup',
...
class PowerMaxStorageGroup(object):
    ...
    def __init__(self):
        ...
        self.module_params\
            .update(get_powermax_storage_group_parameters())
    ...
    def get_volume(self):
        ...
        try:
            ...
            LOG.error('Got error %s while getting details '
                      'of volume %s', str(e), vol_id)
            return None
    ...
    def get_volumes_storagegroup(self, sg_name):
        ...
        try:
            ...
            sg_vol_list = self.provisioning\
                .get_vols_from_storagegroup(sg_name)
    ...
    def get_storage_group(self, sg_name):
        ...
        try:
            ...
            LOG.info('Got error %s while getting details '
                     'of storage group %s', str(e), sg_name)
            return None
    ...
    def create_storage_group(self, sg_name):
        ...
        try:
            ...
            LOG.error('Failed to create storage group %s with error %s',
                      sg_name, str(e))
            self.module.fail_json(msg='Create storage group {0} failed.'
                                      '{1}'.format(sg_name, str(e)))
    ...
    def add_volume_storage_group(self, sg_name):
        ...
        for vol in volumes:
            ...
            if ('vol_id' in vol) and ('vol_name' in vol):
                LOG.warn('Both name and id are found for volume '
                         '%s. No action would be taken. '
                         'Please specify either name or id', vol)
                continue
            if ('cap_unit' in vol) or ('vol_name' in vol) \
                    or ('size' in vol):
                if 'cap_unit' in vol:
                    unit = vol['cap_unit']
                else:
                ...
                if 'vol_name' in vol:
                    ...
                    error_message = 'No valid name was specified ' \
                                    'for the volume {0}'.format(vol)
                    LOG.error(error_message)
                    self.module.fail_json(msg=error_message)
                if 'size' in vol:
                    size = vol['size']
                else:
                    LOG.error('No valid size was specified for '
                              'the volume %s', vol)
                    self.module.fail_json(msg='No valid size was '
                ...
                try:
                    ...
                    if not volume_list or len(volume_list) == 0:
                        LOG.info('No volume found with volume '
                                 'identifier %s in storage group %s',
                                 name, sg_name)
                        LOG.info('Creating volume with name %s, '
                                 'size %s %s', name, size, unit)
                        self.provisioning \
                            .create_volume_from_sg_return_dev_id(name,
                                                                 sg_name,
                                                                 size,
                                                                 unit)
                        changed = True
                        ...
                        for volume in volume_list:
                            vol_details = self.provisioning \
                                .get_volume(volume)
                            if unit == 'GB' \
                                    and vol_details['cap_gb'] == size:
                                break
                            if unit == 'MB' \
                                    and vol_details['cap_mb'] == size:
                                break
                            if unit == 'TB' \
                                    and vol_details['cap_gb'] == \
                                    utils.get_size_in_gb(size, unit):
                                break
                            error_message = 'A volume with identifier {0} ' \
                                            'but different size {1} GB ' \
                                            'already exists. Please use a ' \
                                            'different identifier for ' \
                                            'volume creation. Currently, ' \
                                            'we support only unique ' \
                                            'identifiers for volume ' \
                                            'creation on PowerMax from ' \
                                            'Ansible'.format(
                                                name,
                                                vol_details['cap_gb'])
                            LOG.error(error_message)
                    ...
                    LOG.error('Create volume for storage group '
                              '%s failed with error %s', sg_name, str(e))
                    self.module.fail_json(msg='Create volume for '
                                              'storage group {0} '
                                              'failed.'.format(sg_name))
        existing_volumes_in_sg = self.provisioning \
            .get_vols_from_storagegroup(sg_name)
    ...
    def delete_storage_group(self, sg_name):
        ...
        try:
            ...
            error_message = 'Delete storage group {0} failed ' \
                            'with error {1} '.format(sg_name, str(e))
            LOG.error(error_message)
            self.module.fail_json(msg=error_message)
    ...
    def add_existing_volumes_to_sg(self, vol_list, sg_name):
        ...
        volumes_to_add = [vol for vol in vol_ids
                          if vol not in set(existing_volumes_in_sg)]
        ...
        if duplicate_names:
            self.module.fail_json(
                msg='One or more volumes to be added are already '
                    'present in SG {0} with the same name.'.
                    format(sg_name))

        if not volumes_to_add:
            LOG.info('The given volumes %s are already present '
                     'in storage group %s ', vol_ids, sg_name)
            return False, existing_volumes_in_sg
        try:
            self.provisioning \
                .add_existing_vol_to_sg(sg_name, volumes_to_add)
            existing_volumes_in_sg = self.provisioning \
                .get_vols_from_storagegroup(sg_name)
            return True, existing_volumes_in_sg
        except Exception as e:
            error_message = 'Add existing volume(s) to storage ' \
                            'group {0} failed with ' \
                            'error {1} '.format(sg_name, str(e))
            LOG.error(error_message)
            self.module.fail_json(msg=error_message)
    ...
    def remove_volumes_from_sg(self, vol_list, sg_name):
        ...
        for vol in vol_list:
            ...
            if 'vol_name' in vol:
                ...
                if not volume_list or len(volume_list) == 0:
                    LOG.info('No volume found with volume identifier %s '
                             'in storage group %s', vol['vol_name'], sg_name)
                elif len(volume_list) > 1:
                    self.module.fail_json(msg='Duplicate volumes found:'
                                              ' There are {0} volume(s)'
                                              ' with the same name {1} '
                                              'in the storage group {2}'
        ...
        volumes_to_remove = [vol for vol in vol_ids
                             if vol in set(existing_volumes_in_sg)]

        if not volumes_to_remove:
            LOG.info('The given volumes %s are not present '
                     'on the storage group %s ', vol_ids, sg_name)
            return False, existing_volumes_in_sg
        try:
            for vol in volumes_to_remove:
                self.provisioning \
                    .remove_vol_from_storagegroup(sg_name, vol)
            vol_list = self.provisioning \
                .get_vols_from_storagegroup(sg_name)
            return True, vol_list
        except Exception as e:
            LOG.error('Remove existing volume from storage '
                      'group %s failed with error %s ', sg_name, str(e))
            self.module.fail_json(msg='Remove existing volume from '
                                      'storage group {0} failed.'
                                  .format(sg_name))
    ...
    def modify_sg_srp(self, storage_group, new_srp):
        ...
        try:
            self.provisioning \
                .modify_storage_group(storage_group, payload)
        except Exception as e:
            LOG.error('Modify attribute SRP of storage group %s '
                      'failed with error %s', storage_group, str(e))
            self.module.fail_json(msg='Modify attribute SRP of storage'
                                      ' group {0} failed.'
    ...
    def modify_sg_slo(self, sg_name, new_slo):
        ...
        if ('slo' in storage_group and storage_group['slo'].lower()
            != self.module.params['service_level'].lower()) \
                or ('slo' not in storage_group
                    and self.module.params[
                        'service_level'].lower() != 'none'):
            try:
                LOG.info('The existing SG SLO is %s and '
                         'the desired SG SLO is %s ',
                         storage_group['slo'],
                         self.module.params['service_level'])
                self.provisioning.modify_storage_group(sg_name,
                                                       payload)
            except Exception as e:
                LOG.error('Modify attribute SLO of storage group %s '
                          'failed with error %s ', sg_name, str(e))
                self.module.fail_json(msg='Modify attribute SLO of '
                                          'storage group {0} failed.'
                                      .format(sg_name))
    ...
    def modify_sg_compression(self, sg_name, new_compression):
        ...
        if 'compression' in storage_group \
                and storage_group['compression'] \
                != self.module.params['compression']:
            try:
                LOG.info('The existing SG compression is %s and the '
                         'desired SG compression is %s',
                         storage_group['compression'],
                         self.module.params['compression'])
                self.provisioning.modify_storage_group(sg_name,
                                                       payload)
            except Exception as e:
                LOG.error('Modify attribute compression of storage '
                          'group %s failed with error %s ', sg_name, str(e))
                self.module.fail_json(msg='Modify attribute SRP of '
                                          'storage group {0} failed.'
                                      .format(sg_name))

    def modify_storage_group_attributes(self, storage_group,
                                        modified_param):
        """Modify the Storage group attributes"""

        changed = False
        if 'modified_srp' in modified_param:
            self.modify_sg_srp(storage_group,
                               modified_param['modified_srp'])
            changed = True

        if 'modified_slo' in modified_param:
            self.modify_sg_slo(storage_group,
                               modified_param['modified_slo'])
            changed = True

        if 'modified_compression' in modified_param:
            self.modify_sg_compression(storage_group,
                                       modified_param[
                                           'modified_compression'])
            changed = True
        ...
        for child_sg in child_sg_list:
            if not self.provisioning \
                ...
                try:
                    ...
                    error_message = 'Adding child SG {0} to parent ' \
                                    'storage group {1} failed with ' \
                                    'error {2} '
                    LOG.error('Adding child SG %s to parent storage '
    ...
    def remove_child_sg_from_parent_sg(self, child_sg_list, parent_sg):
        ...
        changed = False
        for child_sg in child_sg_list:
            if self.provisioning.is_child_sg_in_parent_sg(child_sg,
                                                          parent_sg):
                try:
                    self.provisioning \
                        .remove_child_sg_from_parent_sg(child_sg,
                                                        parent_sg)
                    changed = True
                except Exception as e:
                    error_message = 'Removing child SG {0} from parent ' \
                                    'storage group {1} failed with ' \
                                    'error {2} '
                    LOG.error('Removing child SG %s from parent storage '
                              'group %s failed with error %s ', child_sg,
                              parent_sg, str(e))
                    self.module.fail_json(msg=error_message
                                          .format(child_sg, parent_sg,
                                                  str(e)))
    ...
    def is_storage_group_modified(self, storage_group):
        """Determine if the desired storage group state is different
        from existing SG
        """
        ...
        LOG.info('Checking if the storage group %s needs '
                 'modification.', storage_group)
        if ('srp' in storage_group
            and self.module.params['srp'] is not None
            and storage_group['srp'].lower()
            != self.module.params['srp'].lower()) \
                or ('srp' not in storage_group
                    and self.module.params['srp'] is not None
                    and self.module.params['srp'].lower() != 'none'):
            modified_param['modified_srp'] \
                = self.module.params['srp']
            modified = True

        if ('slo' in storage_group
            and self.module.params['service_level'] is not None
            and storage_group['slo'].lower()
            != self.module.params['service_level'].lower()) \
                or ('slo' not in storage_group
                    and self.module.params['service_level'] is not None
                    and self.module.params['service_level'].lower()
                    != 'none'):
            modified_param['modified_slo'] \
                = self.module.params['service_level']
            modified = True
        ...
        if 'compression' in storage_group \
                and self.module.params['compression'] is not None \
                and storage_group['compression'] \
                != self.module.params['compression']:
        ...
        LOG.info('The storage group %s needs to be modified '
                 'and the modified param are %s ', storage_group,
                 modified_param)
    ...
    def rename_storage_group(self, storage_group, new_sg_name):
        ...
        try:
            self.provisioning. \
                modify_storage_group(storage_group['storageGroupId'],
                                     payload)
            changed = True
        except Exception as e:
            error_message = 'Rename storage group {0} to new name {1}' \
                            ' failed with error {2} '
            LOG.error('Rename storage group %s to new name %s '
                      'failed with error %s ',
    ...
    def check_task_validity(self, volumes, vol_state):
        if volumes and vol_state is None:
            self.module.fail_json(msg='Please provide a valid '
                                      'vol_state.')

    def perform_module_operation(self):
        """Perform different actions on SG based on user parameter
        chosen in playbook
        """
        ...
        LOG.debug('Module %s has been called..',
                  __file__)
        ...
        if storage_group:
            modified, modified_param = \
                self.is_storage_group_modified(storage_group)
        ...
        if state == 'present' and storage_group and not new_sg_name \
                and not volumes:
            LOG.info('Get volumes for storage group %s', sg_name)
            result['storage_group_volumes'] \
                = self.get_volumes_storagegroup(sg_name)
        elif state == 'present' and not (
                storage_group or new_sg_name):
            LOG.info('Creating storage group %s', sg_name)
            result['create_sg'], result['storage_group_details'] \
                = self.create_storage_group(sg_name)
            LOG.info('Get volumes for storage group %s', sg_name)
            result['storage_group_volumes'] \
                = self.get_volumes_storagegroup(sg_name)
        ...
        if state == 'present' and vol_state == 'present-in-group' \
            ...
            result['add_new_vols_to_sg'], \
                result['storage_group_volumes'] \
                = self.add_volume_storage_group(sg_name)
            LOG.info('Add existing volume(s) to storage group %s', sg_name)
            result['add_vols_to_sg'], result['storage_group_volumes'] = \
                self.add_existing_volumes_to_sg(volumes, sg_name)
        elif state == 'present' and vol_state == 'absent-in-group' \
                and storage_group and volumes:
            LOG.info('Remove existing volume(s) from '
                     'storage group %s', sg_name)
            result['remove_vols_from_sg'], result['storage_group_volumes'] = \
                self.remove_volumes_from_sg(volumes, sg_name)

        if state == 'present' and storage_group and modified:
            LOG.info('Modifying Storage group %s', sg_name)
            result['modify_sg'] = \
                self.modify_storage_group_attributes(sg_name, modified_param)
            result['storage_group_volumes'] = \
                self.get_volumes_storagegroup(sg_name)
        ...
        if state == 'present' and child_sg_state == 'present-in-group' \
            ...
            result['add_child_sg'] = \
                self.add_child_sg_to_parent_sg(child_sgs, sg_name)
            LOG.info('Get volumes for storage group %s', sg_name)
            result['storage_group_volumes'] = \
                self.get_volumes_storagegroup(sg_name)
        elif state == 'present' and child_sg_state == 'absent-in-group' and \
                storage_group and child_sgs:
            LOG.info('Removing child SG from parent storage '
                     'group %s', sg_name)
            result['remove_child_sg'] = \
                self.remove_child_sg_from_parent_sg(child_sgs, sg_name)
            LOG.info('Get volumes for storage group %s', sg_name)
            result['storage_group_volumes'] = \
                self.get_volumes_storagegroup(sg_name)
        ...
        if state == 'present' and new_sg_name:
            ...
            result['rename_sg'] = \
                self.rename_storage_group(storage_group, new_sg_name)
            if result['rename_sg']:
                sg_name = new_sg_name
                LOG.info('Get volumes for storage group %s', sg_name)
                result['storage_group_volumes'] = \
        ...
        if result['create_sg'] or result['modify_sg'] \
                or result['add_vols_to_sg'] \
                or result['remove_vols_from_sg'] \
                or result['add_child_sg'] \
                or result['remove_child_sg'] or result['delete_sg'] \
                or result['add_new_vols_to_sg'] or result[
                'rename_sg']:
            result['changed'] = True
...
def get_powermax_storage_group_parameters():
    return dict(
        ...
        child_sg_state=dict(required=False,
                            choices=['absent-in-group',
                                     'present-in-group'],
                            type='str'),
