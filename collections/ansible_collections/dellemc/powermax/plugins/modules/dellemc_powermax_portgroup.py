...
description:
- Managing port groups on PowerMax Storage System includes create
  port group with set of ports, add/remove single or multiple ports
  to/from port group, rename port group and delete port group
...
- Ashish Verma (@vermaa31) <Ashish.Verma1@emc.com>
- Rajshree Khare (@kharer5) <rajshree.khare@dell.com>

options:
  portgroup_name:
    description:
    - The name of the port group. No Special Character support
      except for _. Case sensitive for REST Calls.
    type: str
    required: true
  ports:
    description:
    - List of Directors and Ports to be added or removed to/from the
      Port Group
    type: list
  new_name:
      description:
      - New name of the port group while renaming. No Special
  ...
  state:
    ...
    - present - indicates that the port group should be present on
                system
    - absent - indicates that the port group should not be present on
               system
    type: str
    required: true
  ...
  port_state:
    ...
    - present-in-group - indicates that the ports should be present on
                         port group object
    - absent-in-group - indicates that the ports should not be present
                        on port group object
    type: str
...
class PowerMaxPortGroup(object):
    ...
    def get_portgroup(self, portgroup_name):
        ...
        try:
            ...
            LOG.error('Got error %s while getting details of port group'
                      ' %s', str(e), portgroup_name)
            return None
    ...
    def create_portgroup(self, portgroup_name):
        ...
        try:
            if port_state and port_state != 'present-in-group':
                self.module.fail_json(msg='Invalid port_state: Ports '
                                          'can only be added while '
                                          'creating portgroup')
            if ports:
                for port in ports:
            ...
            self.provisioning.create_multiport_portgroup(portgroup_name,
                                                         ports)
            return True
        except Exception as e:
            LOG.error('Failed to create port group %s with error %s',
                      portgroup_name, str(e))
            self.module.fail_json(msg='Create port group {0} failed;'
                                      ' error {1}'
    ...
    def _get_add_ports(self, existing, ports):
        ...
        if ports is not None:
            add_ports = {key: ports[key]
                         for key in ports
                         if key in existing
                         and ports[key] != existing[key]
                         }
    ...
    def _get_rem_ports(self, existing, ports):
        rem_ports = []
        if ports is not None:
            rem_ports = {key: ports[key]
                         for key in ports
                         if key in existing
                         and ports[key] == existing[key]
                         }
            LOG.debug('%s Ports to be removed from port group',
                      rem_ports)
        return rem_ports
    ...
    def add_ports_to_portgroup(self, portgroup_name):
        ...
        if add_ports is None or len(add_ports) == 0:
            LOG.info('No Ports to be added to port group %s', portgroup)
            self.module.fail_json(msg='List of ports to be added is '
        ...
        try:
            ...
            LOG.error('Add port %s to port group %s failed with'
                      ' error %s', port, portgroup_name, str(e))
            self.module.fail_json(msg='Add port {0} to port group {1} '
                                      'failed with error {2}.'
    ...
    def remove_ports_from_portgroup(self, portgroup_name):
        ...
        if rem_ports is None or len(rem_ports) == 0:
            LOG.info('No ports to remove from port group %s',
                     portgroup)
            self.module.fail_json(msg='List of ports to be removed '
                                      'is empty for portgroup {0}'
                                  .format(portgroup))
        try:
            """Need to iterate through all the input port parameters
            as there is no update method in SDK to change them at once.
            ...
            for port in rem_ports:
                ...
                # Need to check whether the port to be removed is
                # present in port-group
            ...
            LOG.error('Remove port %s from port group %s failed with'
                      ' error %s', port, portgroup_name, str(e))
            self.module.fail_json(msg='Remove port {0} from port group'
                                      ' {1} failed with error {2}.'
                                  .format(port, portgroup_name, str(e)))

    def delete_portgroup(self, portgroup_name):
        """Delete port group from system
        A port group cannot be deleted if it is associated with
        a masking view.
        """
        ...
        try:
            ...
            LOG.error('Delete port group %s failed with error %s ',
                      portgroup_name, str(e))
    ...
    def modify_portgroup(self, portgroup_name):
        ...
        if len(new_name) == 0:
            ...
            error_msg = 'Please provide new name for the port group ' \
                        '{0}'.format(portgroup_name)
            LOG.error(error_msg)
        ...
        try:
            ...
            LOG.error('Modifying port group %s failed with error %s',
                      portgroup_name, str(e))
            self.module.fail_json(msg='Modify port group {0} failed '
                                      'with error {1}.'
                                  .format(portgroup_name, str(e)))

    def perform_module_operation(self):
        """Perform different actions on port group based on user
        ...
        if state == 'present' and port_state == 'present-in-group' \
           and portgroup:
            LOG.info('Adding ports to port group %s', portgroup_name)
            changed = self.add_ports_to_portgroup(portgroup_name) \
                or changed

        elif state == 'present' and port_state == 'absent-in-group' \
                and portgroup:
            LOG.info('Removing ports from port group %s',
                     (portgroup_name))
            changed = \
                self.remove_ports_from_portgroup(portgroup_name) \
                or changed
...
def get_powermax_portgroup_parameters():
    ...
    return dict(
        ...
        state=dict(choices=['present', 'absent'], required=True,
                   type='str'),
        port_state=dict(choices=['present-in-group', 'absent-in-group'],
